...
const int COLS=3;//2D array cols for scoreboard
//Function Prototypes
void term();//Exit function
void single(int);//Helps calculate single
bool isDbl(int);//Tests if double
bool isTpl(int);//Tests if triple
void dpay(int &, int dbl=2);//Calculate double pay
void tpay(int &, int tpl=3);//Calculates triple pay
void view(vector <int>);//Forms half of game board
void model (vector <int>);//prints half of game board
void viewA(int []);//forms other half of game board
void modelA (int []);//Prints other half of game board
...
int main(int argc, char** argv) {
   ...
   ofstream output;                     //File output
    ...
    const int SIZE=5;
    vector <int> firs (SIZE);//Half of game board
    firs[0]=1;
    firs[1]=2;
    firs[2]=3;
    int sec[SIZE];  //Other half of game bpard
    sec[0]=4;
    sec[1]=5;
    sec[2]=6;
    float lead[SIZE][COLS]; //2D Array for score board
    ...
    if((num==a1s||num==a2s||num==a3s||num==a4s||num==a5s||num==a6s)&&
        ...
        cout<<"You win by Triple! - Triple the payout"<<endl;
        test=true;
        place=3;
        isTpl(place);
        if (isTpl(place)==true){        //Tests if it is a triple
            tpay(bet);
            cout<<"You won $"<<bet<<endl;       //Calculates winnings if triple
            cout<<endl;
    }
    }
    //if the players number is the same as Die 1 and 2 its double payout
        ...
        place=2;
        isDbl(place);
        if (isDbl(place)==true){            //Tests if it is a double
            dpay(bet);
            cout<<"You won $"<<bet<<endl;       //Calculates winnings if double
            cout<<endl;
        } 
    }
    ...
    //If the players number is the same as any of die 1,2,or 3
        ...
        cout<<"You won $"<<bet<<endl;  //Calculates winnings when it is a single
        cout<<endl;
    }
    }
    else{      //If players number doesn't match any die they lose
         cout<<"You Lose"<<endl;             //Calculates lost money
         test=false;
         cout<<"Pay the dealer $"<<ogbet<<endl;
         cout<<endl;
     }
    ...
    lead[0][0]=games;                   //2D Array increments when others increment
    lead[0][1]=wins;
    lead[0][2]=lose;
    ...
    cout<<"You have lost "<<round(percL)<<"% of games"<<endl;   //% of losses rounded to near percent
     ...
     //outputs the gameboard of games played, wins, and losses
     cout<<setw(20)<<"Games Played "<<setw(12)<<"Games Won "<<setw(12)<<" Games Lost"<<endl;
    cout<<setw(15)<<lead[0][0]<<setw(12)<<lead[0][1]<<setw(12)<<lead[0][2]<<endl;
    ...
    output<<setw(20)<<"Games Played "<<setw(12)<<"Games Won "<<setw(12)<<" Games Lost"<<endl;
    output<<setw(15)<<lead[0][0]<<setw(12)<<lead[0][1]<<setw(12)<<lead[0][2]<<endl;
...
void view(vector<int> num){ 
   cout<<setw(8)<<num[0]<<setw(8)<<num[1]<<setw(8)<<num[2];
}

void model (vector<int> num){
    num[0]=1;
    num[1]=2;
    num[2]=3;
}

void modelA(int numA[]){ 
    numA[0]=1;
    numA[1]=2;
    numA[2]=3;
}
...
void viewA(int numA[]){ 
   cout<<setw(8)<<numA[0]<<setw(8)<<numA[1]<<setw(8)<<numA[2];
}